...
1. Claim a domain name
    - In LearningSuite go to IT&C 210A and select `Content > DNS Registrar > IT&C 210 DNS Registrar`.
    - Click `Add Record`.
...
- Hint: Certbot allows you to find the path to all your LetsEncyrpt certificates and keys with the following command: 
    ```sh
...
This is just an example walkthorugh. # 1-3 will be completed already by following steps 1-3 of this lab. We are doing # 4-5 in this next section.
    1. you have a domain zurich.itc210.net
    2. create an A Record for zurich.itc210.net
    3. have the domain of zurich.itc210.net certified
    4. create a second domain vienna.itc210.net
    5. create a CNAME Record for vienna.itc210.net which maps to zurich.itc210.net


### Creating CNAME record example
![CNAME creation](CNAMEex2.png)
In the example above the `Domain` field is where you will insert a claimed domain and new subdomain. The `Target` field is where you specify the full URL of the domain you want to reference.
...
1. On AWS create another Instance to use your bash script on. follow the same set up as in lab 1a in `Setting up the Live Server on AWS` until you reach the section called `Install Apache2`. This will take you to the step where you are logged into the machine. 
...
- This is what your bash script should do
    ...
    - include all of the commands in step 2
...
- Note: you will need to submit a copy of your bash script thorugh github after this step is complete for the submission.
...
For your submission you will take several screenshots and upload them to learning suite under the `Lab 2b`
1. Image of your certificate found by clicking next to your domain on your website - clicking connection is secure - certificate is valid. It will look something like this. ![](passoffimg1.png)

Make sure that you take a screenshot of both of your domain certificates for both of your instances. 

2. Include a screenshot of your `DNS Records` ![](passoffimg2.png)

3. Submit link to your <lab-2b-username> github
    - Copy your setup_server.sh bash script over to your /var/www/<lab-2b-yourUsername> 
    - Add the file to git, then commit the file to your repository and push. 
    - verify in the browser that the file is in github.
...
- [ ] 10 points - You have a TLS certificate on your website for both domains and you can access it using https://yourdomain.itc210.net.
