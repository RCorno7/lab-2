...
#### Setting Up Your Backup Instance


1. On AWS, **create another Server Instance to use your bash script on**. **Do ***NOT*** run the bash script on your main instance.** This will likely ruin your current configuration. 

> Note: It is important, and best practice, to name your instances so that you remember which one is which. Many students have spent hours trying to resolve odd errors with their script only to realize they ran it in the wrong instance.

2. Follow the same set up as in lab 1a in `setting_up_aws_live_server.md` until you reach the section called `Install Apache2`. Stop there. This will take you to the step where you are logged into the machine. 
3. Once you are logged in create your bash script and name it something like `setup_server.sh` in your main folder. This script will contain the instructions from `setting_up_aws_live_server.md` as well as the steps to get a TLS Certificate as shown in `Step 2` of this lab. The `domain` that you will use for this script will be a new domain with a corresponding `A Record`, this way you will have 2 domains with `TLS certificates`, as well as a CNAME to the first domain.
